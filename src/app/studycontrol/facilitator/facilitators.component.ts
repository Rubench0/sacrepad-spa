...
/**
 * Componente de control de estudio que permite visualizar los facilitadores.
 *
 * @export
 * @class FacilitatorsComponent
...
export class FacilitatorsComponent implements AfterViewInit, OnInit {
	 ...
	 * @type {User} facilitators - Instacia del objeto User.
	 ...
	 * @memberOf FacilitatorsComponent
	 */
	public title: string;
	public token: string;
	public identity: any;
	...
	public dtTrigger: Subject<FacilitatorsComponent> = new Subject();
	...
	ngAfterViewInit() {
		this.renderer.listenGlobal('document', 'click', (event) => {
			if (event.target.hasAttribute('view-user-id')) {
				this._router.navigate(['/studycontrol/facilitator/edit', event.target.getAttribute('view-user-id')]);
			}
		});
	}
}
