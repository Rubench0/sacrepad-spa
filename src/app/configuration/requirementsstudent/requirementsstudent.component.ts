...
/**
 * Componente de configuración que permite visualizar los requerimientos de estudiantes.
 *
 * @export
 * @class RequirementStudentComponent
 * @implements {AfterViewInit}
 * @implements {OnInit}
 */
@Component({
	selector: 'requirementstudent-views',
	templateUrl: '../modelconfiguration.html',
	providers: [UserServices,ConfigurationServices]
})

export class RequirementStudentComponent implements AfterViewInit, OnInit {
	 ...
	 * @memberOf RequirementStudentComponent
	 */
	public title: string;
	...
	public dtTrigger: Subject<RequirementStudentComponent> = new Subject();
	...
	ngOnInit() {
		if (this.identity == null) {
			...
			this._configurationService.viewsrequiremetStudent().subscribe(
				(response:any) => {
	...
	ngAfterViewInit() {
		this.renderer.listenGlobal('document', 'click', (event) => {
			if (event.target.hasAttribute('view-id')) {
				this._router.navigate(['configuration/requirementstudents/edit', event.target.getAttribute('view-id')]);
			}
		});
	}
