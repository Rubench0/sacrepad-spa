...
/**
 * Componente de seguridad para abrir y cerrar sesión de usuario en el sistema.
 *
 * @export
...
export class LoginComponent implements OnInit {
	...
	/**
	 * @method logout
	 * @description Método para cerrar sesión de usuario.
	 * Se utiliza el servicio _route para saber cual es el id de usuario y se procede a borrar del localStorage la información de sesión, una vez borrada se redirecciona a la ruta /login.
	 *
	 * @memberOf LoginComponent
	 */
	...
	logout() {
		this._route.params.forEach((params: Params) => {
			...
			if (logout == 1) {
				...
				this.identity = null;
				this.token = null;
				window.location.href = '/sacrepad-spa/';
			}
		});
	}
	...
	/**
	 * @method onSubmit
	 * @description Permite iniciar sesión de usuario.
	 * Suscribe al servicio _userService para verificar si los datos son correctos, primero traer los datos del usuario para agregarlos al LocalStorage y luego trae el token de acceso, retornando a la vista principal del sistema de ser exitoso.
